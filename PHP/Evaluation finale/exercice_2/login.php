...
$contenu ='';
    ...
    if(!empty($contenu)) {
        ...
        $result = $db->prepare("SELECT * FROM inscription WHERE email = :email AND password = :password AND type = :type",
                                        array(':email' => $_POST['email'], ':password' => $_POST['password']));
        if($result->rowCount() != 0){  // Si il y a un enregistrement dans le resultat, c'est que l'email et mdp correspondent.
            $membre = $result->fetch(PDO::FETCH_ASSOC);  // Pas de while car il y a unicité de l'email.

            header('http://wf3.apolearn.com/');

        }elseif($_POST['type'] == 'formateur'){
            header('http://www.wf3.fr/lecole/lequipe-pedagogique');
        }else{
