...
$queryUsers = $db->prepare('SELECT * FROM users'.$order);
    ...
    if(!empty($_POST)){
        ...
        if(count($errors) == 0){
            ...
            $insertUser = $db->prepare('INSERT INTO users (gender, firstname, lastname, email, birthdate, city)
...
<html>
    ...
    <?php
        if(isset($createUser) && $createUser == true){
            ...
            echo '<div class="alert alert-success">Le nouvel utilisateur a été ajouté avec succès.</div><br>';
        }

        if(!empty($errors)){
            echo '<div class="col-md-6 col-md-offset-3">';
            echo '<div class="alert alert-danger">'.implode('<br>', $errors).'</div><br>';
        }
    ?>
        ...
        <div class="col-md-7">
            <table class="table">
                ...
                <tbody>
                    <?php foreach($_POST as $users => $user)?>
                    <tr>
                        <td><?php echo $user['gender'];?></td>
                        <td><?php echo $user['firstname'];?></td>
                        <td><?php echo $user['lastname'];?></td>
                        <td><?php echo $user['email'];?></td>
                        <td><?php echo DateTime::createFromFormat('Y-m-d', $user['birthdate'])
                                ->diff(new DateTime('now'))->y;?> ans</td>
                    </tr>
                </tbody>
            </table>
        </div>
